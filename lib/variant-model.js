  ...
  respondWithFile: function(options) {
    ...
    return this.respondWith(function (request, reply) {
      ...
      if (options.path) {
        ...
        fs.readFile(options.path, function(err, stream) {
          if (err) {
            if (err.code === 'ENOENT') {
              // doesn't exist
              return reply(options.path + ' not found').code(404);
            } else {
              return reply(err);
            }
          }
          var mimeType = mimeTypes.lookup(options.path);
          reply(stream).type(mimeType).code(options.code || 200);
        });
      } else {
        // a specific handler must be provided
        var initOptions = require('./index').initOptions;
        var handlerFunction = initOptions.replyWithFileHandler;
        if (!handlerFunction) {
          return reply({
