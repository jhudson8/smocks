...
_.extend(Route.prototype, {
  ...
  applyProfile: function(profile, request) {
    this.resetRouteConfig(request);
    var routeConfig = this._mocker.state.routeState(request);
    routeConfig[this._id]._activeVariant = profile.activeVariant;

    this.resetSelectedConfig(request);
    var selectedConfig = this.selectedConfig(request);
    _.extend(selectedConfig, profile.selections && profile.selections.route);

    _.each(this.variants(), function(variant) {
      variant.resetSelectedConfig(this, request);
      var selections = profile.selections && profile.selections.variants && profile.selections.variants[id];
      if (selections) {
        variant.selectConfig(selections, this, request);
      }
    }, this);
  },
