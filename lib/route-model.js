  ...
  this.actions = {
    ...
    execute: function(id, input, request) {
      ...
      if (!action) {
        ...
        var context = smocks.context = smocks._executionContext(request, self);
        var rtn =  action.handler.call(context, input);
...
_.extend(Route.prototype, {
  ...
  getDisplayValue: function(request) {
    if (this._display) {
      var smocks = require('./index');
      var context = smocks.context = smocks._executionContext(request, this);
      var rtn = this._display.call(context);
      delete smocks.context;
      return rtn;
    }
  },
  ...
  resetSelectedInput: function(request) {
    ...
    function setDefaultValue(key, data, obj) {
      var smocks = require('./index');
      var context = smocks.context = smocks._executionContext(request, route);
      var value = _.isFunction(data.defaultValue) ? data.defaultValue.call(context) : data.defaultValue;
      delete smocks.context;
      if (value !== undefined) {
        obj[key] = value;
      }
