...
var Route = function (data, smocks) {
  this._smocks = smocks;
  ...
  this._websocketId = data.websocketId || false;
  ...
  var _actions = this._actions = data.actions || {};
...
_.extend(Route.prototype, {
  ...
  websocketId: function () {
    return this._websocketId || this._path;
  },
  ...
  getDisplayValue: function (request, context) {
    if (this._display) {
      var smocks = require('./index');
      return this._display.call(context);
    }
  },
  ...
  variants: function () {
    ...
    _.each(this._smocks.variants.get(), function (variant) {
      if (!index[variant.id()]) {
  ...
  getVariant: function (id) {
    ...
    return this._smocks.variants.get(id);
  },
  ...
  selectVariant: function (id, request) {
    return util.executionContext({
      ...
      _.each(this._variants, function (variant) {
        if (variant.id() === id) {
          match = true;
          variant.onActivate && variant.onActivate.call(context);
        }
      });
      ...
      if (!match) {
        _.each(this._smocks.variants.get(), function (variant) {
          if (variant.id() === id) {
            match = true;
            variant.onActivate && variant.onActivate.call(context, this);
          }
        }, this);
      }

      if (match) {
        this._smocks.state.routeState(request)[this._id]._activeVariant = id;
      } else {
  ...
  plugin: function (plugin) {
    return this._smocks.plugin(plugin);
  },
  ...
  activeVariant: function (request) {
    return this._smocks.state.routeState(request)[this._id]._activeVariant;
  },
  ...
  config: function (config) {
    if (config) {
      ...
      var others = {};
      if (this._websocketId) {
        others.id = this._websocketId;
      }
      return _.assign({}, this._config, others);
    }
  },
  ...
  resetRouteVariant: function (request) {
    var routeState = this._smocks.state.routeState(request)[this.id()] = {};
  ...
  resetSelectedInput: function (request) {
    var rootInput = this._smocks.state.routeState(request)[this.id()];
    if (!rootInput) {
      this._smocks.state.routeState(request)[this.id()] = {};
      rootInput = this._smocks.state.routeState(request)[this.id()];
    }
    ...
    util.executionContext({
      ...
      function setDefaultValue(key, data, obj) {
        var value = typeof data.defaultValue === 'function' ? data.defaultValue.call(context) : data.defaultValue;
        if (value !== undefined) {
          obj[key] = value;
        }
      }
  ...
  selectRouteInput: function (selectedInput, request) {
    var input = this._smocks.state.routeState(request);
  ...
  selectedVariantInput: function (variant, request) {
    var input = this._smocks.state.routeState(request)[this.id()]._variantInput;
    if (!input) {
      input = this._smocks.state.routeState(request)[this.id()]._variantInput = {};
    }
  ...
  selectVariantInput: function (selectedInput, variant, request) {
    var input = this._smocks.state.routeState(request)[this.id()];
  ...
  _handleRequest: function (request, reply, server, context) {
    var self = this;
    var smocks = this._smocks;
    var variant = this.getActiveVariant(request);

    if (variant) {
      if (variant.handler) {
        return variant.handler.call(context, request, reply, server);
      } else {
  ...
  route: function (data) {
    ...
    return this._smocks.route(data);
  }
});
