...
_.extend(Route.prototype, {
  ...
  resetSelectedConfig: function(request) {
    ...
    var selectedRouteConfig = rootConfig._config = {};
  ...
  getConfigValue: function(id, request) {
    ...
    return variantConfig[id];
  },

  selectedVariantConfig: function(variant, request) {
    var config = this._mocker.state.routeState(request)[this.id()]._variantConfig;
    if (!config) {
      config = this._mocker.state.routeState(request)[this.id()]._variantConfig = {};
    }
    config = config[variant.id()];
    if (!config) {
      config = config[variant.id()] = {};
    }
    return config;
