...
_.extend(Route.prototype, {
  ...
  _handleRequest: function(request, reply) {
    ...
    if (variant) {
      return variant.responder.call(executionContext(this, request), request, reply);
    } else {
  ...
  selectVariant: function(id, request) {
    ...
    _.each(this.variants, function(variant) {
      if (variant.variant.id === id && variant.onActivate) {
...
function executionContext(route, request) {
  return {
    state: function(id, value) {
      if (value !== undefined) {
        route.mocker.state.get(request)[id] = value;
      } else {
        return route.mocker.state.get(request)[id];
      }
    },
    config: function(id) {
