...
var AdminPage = React.createClass({
  ...
  render: function() {
    ...
    var _routes = _.compact(_.map(this.props.routes, function(route) {
      if (filter && route.path.indexOf(filter) === -1) {
        return;
      }
    ...
    return React.DOM.div({},
      React.DOM.div({className: 'action-group'},
        React.DOM.button({type: 'button', className: 'btn btn-default', onClick: this.resetState}, 'Reset State')
      ),
  ...
  render: function() {
    ...
    if (this.props.selected) {
      ...
      return React.DOM.div({className: 'route route-active'},
        React.DOM.button({ className: 'route-title', onClick: this.onDeselect }, '(' + route.method + ') ' + route.path),
        React.DOM.div({ className: 'route-body' },
          React.DOM.h5({},
            'Select a response handler ',
            route.method === 'GET' ? ['( ', React.DOM.a({ key: 'link', href: route.path }, 'view endpoint'), ' )'] : undefined
          ),
      ...
      return React.DOM.div({className: 'route route-passive'},
        React.DOM.button({ className: 'route-title', onClick: this.onSelect }, '(' + route.method + ') ' + route.path, React.DOM.small({}, '(' + selectedVariant.id + ')'))
      );
    }
  },
