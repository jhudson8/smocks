...
var api = {
  ...
  loadProfile: function(profile) {
    if (profile.type === 'remote') {
      this.doPost('/profile/' + encodeURIComponent(profile.name), {}, 'POST', function() {
        alert('profile "' + profile.name + '" loaded');
      });
    } else {
      profile = this.getLocalProfile(profile.name);
      if (profile) {
        this.doPost('/profile', profile, 'POST', function() {
    ...
    if (this.props.selected) {
      ...
      return React.DOM.div({className: 'route route-active'},
        ...
        React.DOM.div({ className: 'route-body' },
          ...
          React.createElement(ConfigList, { data: activeVariant.config, values: route.selections && route.selections.variants[activeVariant.id],
