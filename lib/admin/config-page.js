  ...
  loadProfile: function(profile) {
    if (profile.type === 'remote') {
      this.doPost('/profile/' + encodeURIComponent(profile.name), {}, 'POST', function(data) {
        ...
        resetConfig(data);
        alert(profile.name + '" was applied');
      });
    } else {
...
var AdminPage = React.createClass({
  ...
  render: function() {
    ...
    if (viewScope === 'fixtures') {
      ...
      body = (
        <div>
          {[<ProfileManager ref="profiles" key={this.state.profileState} selected={selectedProfile} remoteConfigComplete={this.remoteProfileConfigComplete}
...
var HarTab = React.createClass({
  ...
  render: function() {
    ...
    if (data.har) {
      return <HarDetails details={data.har}/>;
    }
                ...
                <div key={index} className="item">
                  ...
                  <div className="content">
                    {call.method === 'GET' && !call.responded ?
                      ...
                      :
                      <div className={className}>{call.path} ({call.method})</div>
                    }
                  </div>
                </div>
              );
