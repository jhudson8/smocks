...
var AdminPage = React.createClass({
  ...
  render: function() {
    ...
    var _routes = _.compact(_.map(this.props.routes, function(route) {
      var filterContent = viewType === 'label' ? (route.label || route.path) : (route.path + ' ' + route.method);
  ...
  render: function() {
    ...
    if (this.props.selected) {
      ...
      return React.DOM.div({className: 'route route-active'},
        React.DOM.a({ href: '#path/' + route.method + '%20' + encodeURIComponent(route.path), className: 'permalink'}, 'permalink'),
