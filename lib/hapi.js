...
module.exports = function (smocks) {
  ...
  Object.assign(smocks, {
    ...
    start: function (hapiOptions, smocksOptions, callback, plugins) {
      if (!smocks.id()) {
      ...
      plugins.push({
        ...
        options: {
          onConnection: function(socket) {
            smocks.websocketConnectionHandlers.run(hapiServer, socket);
          },
          onDisconnection: function(socket) {
            smocks.websocketDisconnectionHandlers.run(hapiServer, socket);
          },
          onMessage: function (socket, message, next) {
            smocks.websocketMessageHandlers.run(hapiServer, socket, message, next);
          }
        }
      });

      hapiServer.register(plugins, function (err) {
  ...
  function wrapReply (request, reply, server, plugins) {
    var rtn = function () {
      var response = reply.apply(this, arguments);
      if (smocks.state.onResponse) {
        smocks.state.onResponse(request, response, server);
      }
      _.each(plugins, function (plugin) {
        if (plugin.onResponse) {
          plugin.onResponse(request, response, server);
        }
      });
      return response;
    };
  ...
  function configServer (server) {
    ...
    var _subsciptions = smocks.subscriptions.get();

    _.each(_subsciptions, function (subscription) {
          ...
          handler: function (request, reply) {
            ...
            function doExecute() {
              if (smocks.state.onRequest) {
                smocks.state.onRequest(request, reply, server);
              }
                ...
                if (plugin) {
                  if (plugin.onRequest) {
                    util.executionContext({
                      ...
                      plugin.onRequest.call(context, request, reply, server, handlePlugins);
                    });
                  } else {
                    handlePlugins();
                  }
                  ...
                  util.executionContext({
                    ...
                    reply = wrapReply(request, reply, server, _plugins);
                    route._handleRequest(request, reply, server, context);
                  });
                }
              }
