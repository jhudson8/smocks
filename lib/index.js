...
var mocker = module.exports = {
  ...
  plugin: function(plugin) {
    ...
    else if (!plugin.respondWith) {
      console.error('A plugin with no respondWith method has been applied');
    }
    ...
    if (route.hasVariants()) {
      ...
      server.route({
        ...
        handler: function(request, reply) {
          if (mocker.state.respondWith) {
            mocker.state.respondWith(request, reply);
          }
          ...
          function handlePlugins() {
            ...
            if (plugin) {
              if (plugin.respondWith) {
                plugin.respondWith.call({
