...
var mocker = module.exports = {
  ...
  plugins: {
    ...
    updateConfig: function(pluginId, id, value) {
      var config = _globalConfigValues;
    ...
    getConfigValue: function(pluginId, id) {
      var config = mocker.globalConfigValues = mocker.globalConfigValues || {};
      var pluginConfig = config[pluginId];
...
function executionContext(request, route, plugin) {
  return {
    ...
    config: function(id) {
      if (plugin) {
        return _globalConfigValues[plugin.id()][id];
      }
