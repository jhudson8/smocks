...
var mocker = module.exports = {
  ...
  plugin: function(plugin) {
    ...
    plugin.id = _.uniqueId('plugin');
    this._plugins.push(plugin);

    if (plugin.config) {
      _.each(plugin.config, function(data, id) {
        this.updatePluginConfig(plugin.id, id, data.defaultValue);
      }, this);
    }
  ...
  updatePluginConfig: function(pluginId, id, value) {
  ...
  getPluginConfigValue: function(pluginId, id) {
    var config = this.globalConfigValues = this.globalConfigValues || {};
    var pluginConfig = config[pluginId];
...
function wrapReply(reply, plugins) {
  var rtn = function() {
    ...
    if (route.hasVariants()) {
      ...
      server.route({
        ...
        handler: function(request, reply) {
          reply = wrapReply(reply, mocker._plugins);
            ...
            if (plugin) {
              if (plugin.onRequest) {
                plugin.onRequest.call({
                  ...
                  config: function(id) {
                    return mocker.getPluginConfigValue(plugin.id, id);
                  },
                  option: function(id) {
